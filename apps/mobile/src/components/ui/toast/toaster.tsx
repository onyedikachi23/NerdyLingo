...
import { toastDefaultColors, toastDefaultValues } from "./constants";
...
const defaultToastOptions = {} as NonNullable<ToasterProps["toastOptions"]>;
export const ToasterUI: React.FC<
	ToasterProps & {
		toasts: ToastProps[];
		setToasts: React.Dispatch<React.SetStateAction<ToastProps[]>>;
		toastsCounter: React.RefObject<number>;
		toastRefs: React.RefObject<Record<string, React.RefObject<ToastRef>>>;
	}
> = ({
			...
			setToasts((currentToasts) => {
				...
				if (shouldUpdate) {
					...
					if (!(newToast.id in toastRefs.current)) {
						toastRefs.current[newToast.id] =
							// eslint-disable-next-line @eslint-react/no-create-ref
							React.createRef<ToastRef>() as React.RefObject<ToastRef>;
					}
	...
	const value = React.useMemo<ToasterContextType>(
		() => ({
			...
			pauseWhenPageIsHidden:
				pauseWhenPageIsHidden ??
				toastDefaultValues.pauseWhenPageIsHidden,
			gap: gap ?? toastDefaultValues.gap,
			theme: theme ?? toastDefaultValues.theme,
			colors: colors ?? toastDefaultColors,
			toastOptions,
			autoWiggleOnUpdate:
